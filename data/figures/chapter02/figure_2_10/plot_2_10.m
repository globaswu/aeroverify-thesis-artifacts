...
if nargin < 1, outputFile=fullfile(fileparts(mfilename('fullpath')),'plot_2_10.png'); end
T=loadSiblingData('figure_2_10.csv');
...
ax=nexttile(tl); scatter(ax,xyz(ref,2),xyz(ref,3),1,[.85 .85 .85],'.'); hold(ax,'on'); scatter(ax,def(take,2),def(take,3),2,mag(take),'filled'); axis(ax,'equal'); title(ax,'Right'); xlabel(ax,'y (m)'); ylabel(ax,'z (m)'); colormap(fig,'turbo'); cb=colorbar(ax); cb.Layout.Tile='east'; cb.Label.String='Normalized mode-3 displacement magnitude'; title(tl,sprintf('FCC case 67 SOL 103 mode 3 (%.6f Hz)',numericSubset(T,'frequency_hz',1))); finishPlot(fig,outputFile);
end
