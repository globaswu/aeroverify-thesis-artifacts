function plot_2_11(outputFile)
%PLOT_2_8 Reproduce thesis Figure 2.11 from the sibling CSV only.
scriptFile = mfilename('fullpath');
figureDirectory = fileparts(scriptFile);
dataFile = fullfile(figureDirectory, 'figure_2_11.csv');
if nargin < 1 || strlength(string(outputFile)) == 0
    outputFile = fullfile(figureDirectory, 'plot_2_11.png');
else
...
assert(all(ismember(required, string(data.Properties.VariableNames))), ...
    'Figure 2.11 CSV has an unexpected schema.');
...
for rowIndex = 1:2
    ...
    assert(height(rows) == 740, ...
        'Expected 740 samples for each Figure 2.11 configuration.');
