function plot_2_13(outputFile)
%PLOT_2_13 Reproduce thesis Figure 2.13 from the sibling CSV only.
scriptFile = mfilename('fullpath');
figureDirectory = fileparts(scriptFile);
dataFile = fullfile(figureDirectory, 'figure_2_13.csv');
if nargin < 1 || strlength(string(outputFile)) == 0
    outputFile = fullfile(figureDirectory, 'plot_2_13.png');
else
...
assert(all(ismember(required, string(data.Properties.VariableNames))), ...
    'Figure 2.13 CSV has an unexpected schema.');
...
for rowIndex = 1:2
    ...
    assert(height(rows) == 740, ...
        'Expected 740 samples for each Figure 2.13 configuration.');
