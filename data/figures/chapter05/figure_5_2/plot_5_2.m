function plot_5_2(outputFile)
if nargin < 1, outputFile=fullfile(fileparts(mfilename('fullpath')),'plot_5_2.png'); end
T=loadSiblingData('figure_5_2.csv');

fig=figure('Visible','off','Color','w'); ax=axes(fig); plotPareto(ax,T,'mass_kg','compliance_Nm','feasible','pareto_case071','case_id'); xlabel(ax,'Lattice-wing mass (kg)'); ylabel(ax,'Trim compliance (N m)'); title(ax,'Completed FCC continuation Pareto record'); legend(ax,'Location','best'); finishPlot(fig,outputFile);
end
