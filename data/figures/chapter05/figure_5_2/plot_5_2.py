...
def score_panel(ax,grid,points,xname,yname,score,px,py,feasname,pfname,title):
    ...
    return image
def parse_output():
    parser=argparse.ArgumentParser(); parser.add_argument("--output",type=Path,default=HERE/"plot_5_2.png")
...
def main():
    d=load_rows(); fig,ax=plt.subplots(figsize=(6.5,4.8),constrained_layout=True)
    pareto_panel(ax,d,"mass_kg","compliance_Nm","feasible","pareto_case071","case_id")
    ax.set_xlabel("Lattice-wing mass (kg)"); ax.set_ylabel("Trim compliance (N m)"); ax.set_title("Completed FCC continuation Pareto record"); ax.legend(frameon=False); finish(fig)
