function plot_5_20(outputFile)
if nargin < 1, outputFile=fullfile(fileparts(mfilename('fullpath')),'plot_5_20.png'); end
T=loadSiblingData('figure_5_20.csv');

feasible=logicalCol(T,'feasible'); pf=logicalCol(T,'final_pareto'); cont=numericCol(T,'case')>=41; specs={{'AR','CDitrim','Aspect ratio','Trim induced-drag coefficient'};{'AR','Ctrim_Nm','Aspect ratio','Trim compliance (N m)'};{'AR','max_screening_stress_mpa','Aspect ratio','Maximum screening stress (MPa)'};{'modeled_half_wing_mass_kg','Ctrim_Nm','Modeled half-wing mass (kg)','Trim compliance (N m)'}}; fig=figure('Visible','off','Color','w','Position',[100 100 850 700]); tl=tiledlayout(fig,2,2,'Padding','compact','TileSpacing','compact'); for i=1:4, ax=nexttile(tl); x=numericCol(T,specs{i}{1}); y=numericCol(T,specs{i}{2}); plotObservations(ax,x,y,feasible); scatter(ax,x(pf),y(pf),32,'filled','MarkerFaceColor',[.13 .13 .13]); scatter(ax,x(pf&cont),y(pf&cont),58,'o','MarkerFaceColor','none','MarkerEdgeColor',[.90 .38 0]); xlabel(ax,specs{i}{3}); ylabel(ax,specs{i}{4}); axis(ax,'square'); end; title(tl,'Observed structural and objective trends'); finishPlot(fig,outputFile);
end
