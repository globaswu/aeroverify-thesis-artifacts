function plot_5_3(outputFile)
if nargin < 1, outputFile=fullfile(fileparts(mfilename('fullpath')),'plot_5_3.png'); end
T=loadSiblingData('figure_5_3.csv');

G=T(textCol(T,'record_type')=="grid",:); P=T(textCol(T,'record_type')=="evaluation",:); fig=figure('Visible','off','Color','w'); ax=axes(fig); h=plotScore(ax,G,P,'a_m','t1_over_a','score_after_observed','a_m','t1_over_a','feasible_observed','pareto_case071'); colormap(ax,'parula'); colorbar(ax); xlabel(ax,'Cell size, a (m)'); ylabel(ax,'Primary-member ratio, t1/a'); title(ax,'Completed FCC feasibility-score map'); finishPlot(fig,outputFile);
end
