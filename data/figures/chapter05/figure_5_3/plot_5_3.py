...
def score_panel(ax,grid,points,xname,yname,score,px,py,feasname,pfname,title):
    ...
    return image
def parse_output():
    parser=argparse.ArgumentParser(); parser.add_argument("--output",type=Path,default=HERE/"plot_5_3.png")
...
def main():
    d=load_rows(); g=select(d,"grid"); p=select(d,"evaluation"); fig,ax=plt.subplots(figsize=(6.3,5.3),constrained_layout=True)
    im=score_panel(ax,g,p,"a_m","t1_over_a","score_after_observed","a_m","t1_over_a","feasible_observed","pareto_case071","Completed FCC feasibility-score map")
    ax.set_xlabel("Cell size, a (m)"); ax.set_ylabel("Primary-member ratio, t1/a"); fig.colorbar(im,ax=ax,label="Binary-feasibility score"); ax.legend(frameon=False); finish(fig)
