...
def score_panel(ax,grid,points,xname,yname,score,px,py,feasname,pfname,title):
    ...
    return image
def parse_output():
    parser=argparse.ArgumentParser(); parser.add_argument("--output",type=Path,default=HERE/"plot_5_4.png")
...
def main():
    d=load_rows(); cats=list(dict.fromkeys(text(d,"plot_category").tolist())); markers=["o","x","+","s","^","D","v"]
    fig,ax=plt.subplots(figsize=(6.3,5.3),constrained_layout=True); x=num(d,"a_m"); y=num(d,"t1_over_a")
    for cat,marker in zip(cats,markers):
        m=text(d,"plot_category")==cat; ax.scatter(x[m],y[m],marker=marker,s=35,label=cat.replace("_"," "))
    pf=flag(d,"pareto_case071"); ax.scatter(x[pf],y[pf],s=65,facecolors="none",edgecolors=INK,label="Observed Pareto")
    ax.set_xlabel("Cell size, a (m)"); ax.set_ylabel("Primary-member ratio, t1/a"); ax.set_box_aspect(1); ax.set_title("FCC observed stress-failure mechanisms"); ax.legend(frameon=False,fontsize=6,ncol=2); finish(fig)
