...
NUMBER = 9
CSV_FILE = SCRIPT.parent / "figure_C_4.csv"
DEFAULT_OUTPUT = SCRIPT.parent / "plot_C_4.png"

PARSER = argparse.ArgumentParser(
    description="Render Thesis Figure C.4 from its sibling CSV."
)
PARSER.add_argument(
    "--output",
    type=Path,
    default=DEFAULT_OUTPUT,
    help="Output image path (default: sibling plot_C_4.png).",
)
...
def plot_conditional(data):
    ...
    # Three panels retain the survival grid, four profiles, and support curves.
    # Explicit line styles and a red selected-design star supplement color.
    ...
    x, thresholds, log_probability = matrix(
        survival, "log10_conditional_exceedance_probability",
        "input_physical", "threshold_normalized")
    _, _, probability = matrix(
        survival, "conditional_exceedance_probability",
        "input_physical", "threshold_normalized")
    physical = column(profile, "input_physical")
    floor_probability = 1 / 2048
    figure = plt.figure(figsize=(7.5, 9.2), constrained_layout=True)
    layout = figure.add_gridspec(3, 2, width_ratios=(1, 0.028))
    axes = [figure.add_subplot(layout[index, 0]) for index in range(3)]
    color_axis = figure.add_subplot(layout[0, 1])
    image = axes[0].pcolormesh(
        x, thresholds, log_probability, shading="auto", cmap="viridis",
        vmin=np.log10(floor_probability), vmax=0)
    contours = axes[0].contour(
        x, thresholds, probability, levels=[0.01, 0.1, 0.5],
        colors="black", linewidths=0.7)
    axes[0].set_yscale("log")
    axes[0].set_ylim(1e-8, 1)
    # Label one interior point per level, without erasing contour segments.
    label_positions = []
    for level, segments in zip(contours.levels, contours.allsegs, strict=True):
        ...
        candidates = path[(path[:, 1] >= 1e-6) & (path[:, 1] < 0.8)]
        if len(candidates) == 0:
            continue
        target_x = x[0] + 0.30 * (x[-1] - x[0])
        anchor = candidates[np.argmin(np.abs(candidates[:, 0] - target_x))]
        label_positions.append(tuple(anchor))
    axes[0].clabel(
        contours, manual=label_positions, inline=False, fmt="%g", fontsize=7)
    axes[0].set_yscale("log")
    axes[0].set_ylim(1e-8, 1)
    axes[0].set_ylabel("Normalized HVI threshold, η [-]")
    axes[0].set_title("A. Conditional HVI survival", loc="left", pad=7)
    colorbar = figure.colorbar(image, cax=color_axis)
    colorbar.set_ticks(np.log10([floor_probability, 1e-3, 1e-2, 1e-1, 1]))
    colorbar.set_ticklabels(["≤1/2048", "1e-3", "1e-2", "1e-1", "1"])
    colorbar.set_label("Conditional exceedance probability [-]", labelpad=8)

    def positive_profile(name):
        values = column(profile, name)
        # Zero cannot be represented on a log axis; leave a gap rather than
        # drawing a spurious line down to an arbitrary positive floor.
        return np.where(values > 0, values, np.nan)

    series = [
        ("p90_normalized_hvi", "90th percentile", "#d5a010", "-", 1.4),
        ("p99_normalized_hvi", "99th percentile", "#e86e17", "-", 1.4),
        ("mean_normalized_hvi", "Conditional mean", "#1769aa", "--", 1.4),
        ("sampled_profile_maximum_normalized_hvi",
         "Sampled profile maximum", "#222222", "-", 1.6),
    ]
    for name, label, color, style, width in series:
        axes[1].plot(physical, positive_profile(name), color=color,
                     linestyle=style, linewidth=width, label=label)
    axes[1].scatter(
        number(metadata_row, "selected_input_physical"),
        number(metadata_row, "selected_hvi_normalized"),
        marker="*", s=80, color="#b33a3a", edgecolors="white",
        linewidths=0.65, clip_on=False, zorder=6, label="Selected design")
    axes[1].set_yscale("log")
    axes[1].set_ylim(1e-8, 1.15)
    axes[1].set_ylabel("Normalized HVI [-]")
    axes[1].set_title("B. Conditional quantiles and profile", loc="left", pad=7)
    axes[1].legend(ncol=2, loc="lower left", framealpha=1)

    axes[2].plot(
        physical, positive_profile("positive_hvi_fraction"),
        color="#e86e17", linewidth=1.6, label="Positive-HVI fraction")
    axes[2].plot(
        physical, positive_profile("mean_normalized_hvi"), "--",
        color="#1769aa", linewidth=1.5, label="Mean HVI / global maximum HVI")
    axes[2].set_yscale("log")
    axes[2].set_ylim(1e-8, 1)
    axes[2].set_ylabel("Conditional domain-volume statistic [-]")
    axes[2].set_title("C. Positive-HVI support and conditional mean", loc="left", pad=7)
    axes[2].legend(loc="lower left", framealpha=1)
    for axis in axes:
        axis.set_xlabel(
            f"{metadata_row['input_name']}, {metadata_row['input_symbol']} [in]")
        axis.set_xlim(x[0], x[-1])
        axis.set_yticks([1e-8, 1e-6, 1e-4, 1e-2, 1])
        axis.grid(which="major", alpha=0.22)
        axis.set_axisbelow(True)
    figure.suptitle(
        f"WB150 iteration-130 conditional sampled HVI versus {metadata_row['input_name']}",
        fontsize=11)
    return figure
