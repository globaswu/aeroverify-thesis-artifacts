...
def score_panel(ax,grid,points,xname,yname,score,px,py,feasname,pfname,title):
    ...
    return image
def parse_output():
    parser=argparse.ArgumentParser(); parser.add_argument("--output",type=Path,default=HERE/"plot_D_1.png")
...
def main():
    d=load_rows(); topo=text(d,"topology"); names=list(dict.fromkeys(topo.tolist()))
    fig,axes=plt.subplots(2,2,figsize=(9.0,7.2),constrained_layout=True)
    for ax,name in zip(axes.flat[:3],names):
        m=topo==name; rows=[d[i] for i in np.flatnonzero(m)]
        pareto_panel(ax,rows,"mass_kg","compliance_Nm","feasible","topology_pareto_case051","case_id"); ax.set_title(name)
    ax=axes.flat[3]; pareto_panel(ax,d,"mass_kg","compliance_Nm","feasible","pooled_pareto_case051","case_id"); ax.set_title("Pooled")
    for ax in axes.flat: ax.set_xlabel("Lattice-wing mass (kg)"); ax.set_ylabel("Trim compliance (N m)")
    fig.suptitle("Observed lattice mass-compliance trade-offs"); finish(fig)
