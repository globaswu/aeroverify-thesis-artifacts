...
DATA = HERE / "figure_D_4.csv"
parser = argparse.ArgumentParser(description="Replot thesis Figure D.4 from its sibling CSV.")
parser.add_argument("--output", type=Path, help="Output image path; defaults beside this script.")
arguments = parser.parse_args()
OUTPUT = (arguments.output if arguments.output is not None else HERE / "plot_D_4.png").resolve()
...
ax.set(xlabel="Taper ratio, lambda [-]",ylabel="log10 normalized HVI threshold",title="A. Conditional HVI survival field")
...
ax.set(xlabel="Taper ratio, lambda [-]",ylabel="Normalized HVI [-]",title="B. Conditional quantiles and profile")
...
ax.set(xlabel="Taper ratio, lambda [-]",ylabel="Conditional domain-volume statistic [-]",title="C. Positive-HVI support and conditional mean")
ax.grid(True,which="both",color="#d9d9d9",lw=.5); ax.legend(fontsize=8)
fig.suptitle("Frozen sampled-HVI field versus taper ratio before evaluation 100",fontweight="bold")
for panel in axes:
...
if len(positive_indices) and positive_indices[-1]+1 < len(profile):
    ...
    axes[1].text(.98,.20,f"All sampled HVI = 0 for lambda >= {zero_start:g}",
