function outputFile = plot_D_5(outputFile)
scriptDir = fileparts(mfilename('fullpath'));
if nargin < 1 || isempty(outputFile)
    outputFile = fullfile(scriptDir, 'plot_D_5.png');
end
outputFile = char(outputFile);
outputDir = fileparts(outputFile);
if ~isempty(outputDir) && ~isfolder(outputDir), mkdir(outputDir); end
dataFile = fullfile(scriptDir, 'figure_D_5.csv');
...
title(tl,'Frozen sampled-HVI field versus primary-member ratio before evaluation 100','FontWeight','bold'); exportgraphics(fig,outputFile,'Resolution',300); close(fig); disp(outputFile);
end
