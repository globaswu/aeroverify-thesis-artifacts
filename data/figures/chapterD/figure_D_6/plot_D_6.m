function outputFile = plot_D_6(outputFile)
scriptDir = fileparts(mfilename('fullpath'));
if nargin < 1 || isempty(outputFile)
    outputFile = fullfile(scriptDir, 'plot_D_6.png');
end
outputFile = char(outputFile);
outputDir = fileparts(outputFile);
if ~isempty(outputDir) && ~isfolder(outputDir), mkdir(outputDir); end
dataFile = fullfile(scriptDir, 'figure_D_6.csv');
...
title(tl,'Frozen sampled-HVI field versus secondary-member ratio before evaluation 100','FontWeight','bold'); exportgraphics(fig,outputFile,'Resolution',300); close(fig); disp(outputFile);
end
