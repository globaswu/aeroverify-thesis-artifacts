...
DATA = HERE / "figure_D_6.csv"
parser = argparse.ArgumentParser(description="Replot thesis Figure D.6 from its sibling CSV.")
parser.add_argument("--output", type=Path, help="Output image path; defaults beside this script.")
arguments = parser.parse_args()
OUTPUT = (arguments.output if arguments.output is not None else HERE / "plot_D_6.png").resolve()
...
ax.set(xlabel="Secondary-member ratio, r2 [-]",ylabel="Conditional domain-volume statistic [-]",title="C. Positive-HVI support and conditional mean")
...
if len(positive_indices) and positive_indices[-1]+1 < len(profile):
    ...
    axes[1].text(.98,.20,f"All sampled HVI = 0 for r2 >= {zero_start:g}",
