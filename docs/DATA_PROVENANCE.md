...
| `data/figures/` numerical packages | thesis figure data and associated plot scripts | stable paths mapped to current numbers; scripts read adjacent CSV data (three tables for Figure 2.5, one for the other numerical packages) |
| `data/figures/` screenshot compositions | genuine nTop viewport crops and recorded composition layout | supplied PNG inputs plus layout CSV; compose the montage without regenerating geometry or simulation results |
