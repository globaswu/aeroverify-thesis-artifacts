...
flutter = readtable(fullfile(root, "data", "flutter", ...
    "harmonized_sol145_results.csv"), TextType="string", ...
    VariableNamingRule="preserve");
flutter.strict_pass = to_logical_column(flutter.strict_pass);
assert(height(flutter) == config.flutter_reassessment.expected_cases);
assert(nnz(flutter.strict_pass) == ...
    config.flutter_reassessment.expected_pass);
assert(all(flutter.maximum_damping <= 0));
assert(nnz(flutter.topology == "BCC") == ...
    config.flutter_reassessment.expected_bcc);
assert(nnz(flutter.topology == "FCC") == ...
    config.flutter_reassessment.expected_fcc);
assert(nnz(flutter.topology == "SC") == ...
    config.flutter_reassessment.expected_sc);
assert(all(flutter.num_flutter_roots == ...
    config.flutter_reassessment.expected_roots_per_case));
assert(all(flutter.rows_per_root == ...
    config.flutter_reassessment.expected_velocities_per_root));
assert(all(flutter.total_rows == ...
    config.flutter_reassessment.expected_rows_per_case));
assert(abs(max(flutter.maximum_damping) - ...
    config.flutter_reassessment.least_negative_maximum_damping) < 1e-15);
...
expectedOutputs = [ ...
    ...
    "flutter_reassessment/flutter_reassessment_summary.csv"; ...
